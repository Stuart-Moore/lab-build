...
if (-not $Finalize) {
	...
	Invoke-Pester -Script "$ModuleBase\Tests" -Show None -OutputFormat NUnitXml -Tag $tag -OutputFile "$ModuleBase\$TestFile" -PassThru | Export-Clixml -Path "$ModuleBase\PesterResults$Tag$PSVersion.xml"
}
else {
